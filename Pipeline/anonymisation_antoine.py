...
from text_recognition_antoine import TextFiltering
...
def main() :
    ...
    print('Text Recognition and NER')
    ti = time.time()
    filtered_boxes_dict = {}
    filtered_boxes_name_dict = {}
    for loc, subscreen in info.subscreens.items() :
        filtered_boxes, filtered_boxes_names = TextFiltering(os.getcwd().replace('\\', '/')+'/models').process_scene(subscreen, info.boxes[loc], info.scene_frames[loc])
        filtered_boxes_dict[loc] = filtered_boxes
        filtered_boxes_name_dict[loc] = filtered_boxes_names
    info.filtered_boxes = filtered_boxes_dict
    info.filtered_boxes_name = filtered_boxes_name_dict
    print(info.filtered_boxes_name)
    print(f'Text Recognition & NER in {time.time()-ti:.3f}s')
    ...
    masked_subscreens = {}
    for loc, subscreen in info.subscreens.items() :
        masked_subscreen = Masker().mask_video(subscreen, info.filtered_boxes[loc], info.scene_frames[loc])
